...
export function StreamCard({stream}: StreamCardProps) {
    return <div className="h-full w-full">
        ...
        <div className="mt-3 flex gap-x-3">
            ...
            <div className="flex flex-items overflow-hidden text-sm">
                <h2 className="flex items-center gap-x-2 font-semibold text-foreground">{stream.user.username}{stream.user.isVerified && <ChannelVerified size='sm' />}</h2>
                {stream.category && (
                    <Link href={`/categories/${stream.category.slug}`} className="text-muted-foreground">{stream.category.title}</Link>
                )}
            </div>
        </div>
    </div>
